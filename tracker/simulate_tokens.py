...
Modos de uso:
1. Interativo (padrão): Janela gráfica onde você clica e arrasta miniaturas com o mouse.
2. Automático (circle/combat): Simula trajetórias e movimentação contínua.

Exemplos:
  python simulate_tokens.py
  python simulate_tokens.py --mode circle --fps 30
  python simulate_tokens.py --mode combat
"""
...
# Mapeamento padrão dos tokens temáticos de Ordem Paranormal
TOKEN_PRESETS = {
    1: {"name": "Investigador 1", "type": "investigador", "color": (255, 200, 50)},   # Ciano/Azul claro
    2: {"name": "Investigador 2", "type": "investigador", "color": (255, 180, 0)},    # Azul
    3: {"name": "Investigador 3", "type": "investigador", "color": (200, 255, 0)},    # Ciano esverdeado
    4: {"name": "Investigador 4", "type": "investigador", "color": (150, 220, 50)},   # Verde água
    5: {"name": "Investigador 5", "type": "investigador", "color": (220, 220, 100)},  # Azul acinzentado
    11: {"name": "Zumbi de Sangue", "type": "criatura_sangue", "color": (30, 30, 230)},        # Vermelho Sangue
    12: {"name": "Esqueleto de Lodo", "type": "criatura_morte", "color": (80, 80, 80)},        # Cinza/Lodo
    13: {"name": "Anomalia Eletrica", "type": "criatura_energia", "color": (230, 100, 200)},   # Magenta/Roxo Energia
    14: {"name": "Existido", "type": "criatura_conhecimento", "color": (0, 215, 255)},         # Dourado Conhecimento
    20: {"name": "Aliado Civil", "type": "npc", "color": (180, 230, 180)},                     # Verde Claro
}


class TokenSimulator:
    def __init__(self, ip: str = DEFAULT_UDP_IP, port: int = DEFAULT_UDP_PORT, fps: int = 30):
        self.ip = ip
        self.port = port
        self.fps = fps
        ...
        # Estado dos tokens simulados: id -> dict
        ...
        # Variáveis para modo interativo GUI
    ...
    def send_telemetry(self):
        """Envia pacote UDP no formato padrão do tracker."""
        active_tokens = [
            {
                "id": t["id"],
                "type": t["type"],
        ...
        try:
            ...
            print(f"[Erro UDP] Falha ao enviar: {e}", file=sys.stderr)

    def run_circle_mode(self):
        """Movimenta tokens em trajetórias circulares e patrulha contínua."""
        ...
        try:
            while True:
                ...
                # Investigador 1: círculo central
                ...
                # Investigador 2: patrulha elíptica
                ...
                # Criatura de Sangue (ID 11): caçada lenta
    ...
    def run_combat_mode(self):
        """Simula um encontro de combate com aproximações, recuos e giros táticos."""
        print(f"\n[Modo Combate Tático] Enviando telemetria para {self.ip}:{self.port} a {self.fps} FPS.")
        print("Pressione Ctrl+C para encerrar.\n")

        # Adiciona criatura de energia e outro investigador
        ...
        try:
            while True:
                ...
                # Investigador 1 mantém distância da Criatura de Sangue
                ...
                # Criatura 13 (Energia) teleporta/glitcha em pulsos rápidos
    ...
    def _mouse_event(self, event, x, y, flags, param):
        ...
        if event == cv.EVENT_LBUTTONDOWN:
            # Checa se clicou sobre algum token existente
            clicked_id = None
            min_dist = 0.05  # raio de toque ~ 5% da tela
            ...
            if clicked_id is not None:
                ...
                # Se clicou no vazio, move o token selecionado para lá
    ...
    def run_interactive_gui(self):
        """Abre janela interativa do OpenCV para arrastar e controlar tokens visualmente."""
        ...
        while True:
            # Cria tela preta / grid tático
            canvas = np.full((self.canvas_h, self.canvas_w, 3), 20, dtype=np.uint8)

            # Desenha grid tático simulado (quadrados de 60px)
            grid_size = 60
            ...
            for tid, t in self.tokens.items():
                ...
                # Indicador de orientação (frente da miniatura)
                dir_len = radius + 12
            ...
            elif key == ord(' '):  # Espaço: toggle ativo
                if self.selected_token_id in self.tokens:
